...
def modify_position(body: PositionModify):
    ...
    """
    Modify existing position in 'orders' table (update qty/SL/target)
    """
    try:
        with sqlite3.connect(DB_PATH, timeout=10) as conn:
            ...
            c.execute("SELECT * FROM orders WHERE username=? AND script=?",
                      (body.username, body.script))
            existing = c.fetchone()

            if existing:
                c.execute("""
                    UPDATE orders
                    SET qty=?, stoploss=?, target=?, updated_at=?
                    WHERE username=? AND script=?
                """, (
                    body.new_qty,
                    body.stoploss,
                ...
                c.execute("""
                    INSERT INTO orders (username, script, qty, stoploss, target, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (
                    body.username,
                    body.script,
